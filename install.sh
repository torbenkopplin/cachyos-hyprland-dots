...
# CONFIG_HOME comes from XDG_CONFIG_HOME and BIN_HOME comes from HOME. Those are
# two different roots, which means overriding HOME does NOT sandbox this script:
# it goes on linking into whatever XDG_CONFIG_HOME says, and on a normal session
# that is the real ~/.config.
#
...
# displaced links were all still there as *.bak-*) and finding the cause was not.
#
# So: if HOME has been overridden and XDG_CONFIG_HOME did not follow it, that is
# an accident every time, and this refuses rather than acting on half of it. A
# deliberate XDG_CONFIG_HOME outside HOME is left alone -- what is caught is the
# mismatch, not the value.
...
if [[ -n ${XDG_CONFIG_HOME:-} && -n $REAL_HOME && $HOME != "$REAL_HOME" \
  ...
  !! refusing to run: HOME and XDG_CONFIG_HOME disagree
     ...
     $HOME as yours. To sandbox a test run, override both:

       HOME=/tmp/sandbox XDG_CONFIG_HOME=/tmp/sandbox/.config ./install.sh
...
do_update() {
    ...
    if (( DRY_RUN )); then
        say "would: git -C ${REPO/#$HOME/\~} pull --ff-only"
...
# Before anything else: the point of --update is that everything below runs
# from the version that was pulled, not the one you invoked.
(( DO_UPDATE ))   && do_update
